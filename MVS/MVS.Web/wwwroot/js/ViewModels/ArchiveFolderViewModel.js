var ArchiveFolderViewModel = null;

function archiveFolderViewModel() {
    self = this;
    ...
    {
        $.ajax({
            url: '/Vault/Archive?handler=Folders',
            type: 'GET',
    ...
    self.updateFolderArchiveStatus = function (archive) {
        $.ajax({
            url: '/Vault/Index?handler=UpdateFolderArchiveStatus',
            type: 'Post',
            headers: { "RequestVerificationToken": $('input[name="__RequestVerificationToken"]').val() },
            data: {
                vaultId: self.selectedFolder().id,
                archive
            },
            success: function (data) {
    ...
    self.deleteFolder = function (archive) {
        $.ajax({
            url: '/Vault/Index?handler=DeleteVault',
            type: 'Post',
            headers: { "RequestVerificationToken": $('input[name="__RequestVerificationToken"]').val() },
            data: {
                vaultId: self.selectedFolder().id
